                ...
                <select name="product_variant_id" class="w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-4 py-2 focus:ring-2 focus:ring-cyan-500">
                    ...
                    @foreach($productVariants as $variant)
                        <option value="{{ $variant->id }}" {{ request('product_variant_id') == $variant->id ? 'selected' : '' }}>
                            {{ $variant->sku }} - {{ $variant->product ? $variant->product->name : 'N/A' }}
                        </option>
                    @endforeach
                </select>
            </div>
        ...
        <table class="w-full">
            <thead class="bg-gray-800 border-b border-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-300">Mã SP (SKU)</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-300">Tên sản phẩm</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-300">Trạng thái</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-300">Số lượng (Nhập/Xuất)</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-gray-300">Số lượng hiện tại</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-800 hover:bg-gray-800 transition">
                    ...
                    <td class="px-6 py-4 text-cyan-400 font-bold">
                        {{ $history->productVariant ? $history->productVariant->sku : 'N/A' }}
                    </td>

                    <td class="px-6 py-4 text-white font-medium">
                        {{ $history->productVariant && $history->productVariant->product ? $history->productVariant->product->name : 'Sản phẩm đã xóa' }}
                    </td>
                        ...
                        @switch($history->type)
                            ...
                            @case('return')
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-green-500/20 text-green-400">
                                    {{ $history->type == 'in' ? 'Nhập kho' : ($history->type == 'purchase' ? 'Mua hàng' : 'Trả hàng') }}
                                </span>
                                @break
                            @case('out')
                            @case('sale')
                                <span class="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-red-500/20 text-red-400">
                                    {{ $history->type == 'out' ? 'Xuất kho' : 'Bán hàng' }}
                                </span>
                                @break
                            @case('adjustment')
                    ...
                    <td class="px-6 py-4 font-bold text-lg">
                        @if(in_array($history->type, ['in', 'purchase', 'return']))
                            <span class="text-green-500 bg-green-500/10 px-2 py-1 rounded">
                                Nhập: {{ $history->quantity }}
                            </span>
                        @elseif(in_array($history->type, ['out', 'sale']))
                            <span class="text-red-500 bg-red-500/10 px-2 py-1 rounded">
                                Xuất: {{ $history->quantity }}
                            </span>
                        @else
                            <span class="text-blue-500 bg-blue-500/10 px-2 py-1 rounded">
                                Điều chỉnh: {{ $history->quantity }}
                            </span>
                        @endif
                    </td>

                    <td class="px-6 py-4 text-white font-bold text-lg">
