    ...
    <form action="{{ route('customers.update', $customer) }}" method="POST"
        ...
        <div>
            ...
            <textarea name="address" rows="3"
                ...
                class="w-full bg-gray-800 text-white border border-gray-700
                       rounded-lg px-4 py-2 focus:ring-2 focus:ring-cyan-500">{{ old('address', $customer->address) }}</textarea>
            @error('address')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <select name="status"
                ...
                <option value="0" {{ old('status', $customer->status) == 0 ? 'selected' : '' }}>Vô hiệu hóa</option>
            </select>
            @error('status')
            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
