...
@section('title', isset($category) ? $category->name : 'Sản phẩm nổi bật')

@section('content')
    @php
        $isCategoryPage = isset($category);
        $baseRoute = $isCategoryPage ? route('category.products', $category->slug) : route('products.featured');
    @endphp
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-10">
            ...
            <div class="mb-6">
                <h1 class="text-3xl md:text-4xl font-bold text-gray-900">
                    {{ $isCategoryPage ? $category->name : '🔥 Sản phẩm nổi bật' }}
                </h1>
                <p class="text-gray-500 mt-2">
                    {{ $isCategoryPage
                        ? 'Khám phá các sản phẩm trong danh mục ' . $category->name . ' tại Smart Store.'
            ...
            <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                    <a href="{{ $baseRoute }}?{{ http_build_query(request()->except(['brand', 'page'])) }}"
                    ...
                    @foreach ($brands as $brand)
                        <a href="{{ $baseRoute }}?{{ http_build_query(array_merge(request()->except('page'), ['brand' => $brand->slug])) }}"
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                ...
                <aside class="xl:col-span-3">
                    <form method="GET" action="{{ $baseRoute }}"
                        ...
                        <div class="flex gap-3 pt-2">
                            ...
                            <a href="{{ $baseRoute }}?{{ http_build_query(request()->except(['price_range', 'page'])) }}"
                ...
                <section class="xl:col-span-9">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm px-4 py-3 mb-5">
                        ...
                        @if ($isCategoryPage || request('category') || request('brand') || request('price_range') || request('sort'))
                            <div class="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-100">
                                @if ($isCategoryPage)
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-full bg-orange-50 text-orange-600 text-sm font-medium">
                                        {{ $category->name }}
                                    </span>
                                ...
                                @if (request('price_range') && request('price_range') !== 'all')
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-full bg-orange-50 text-orange-600 text-sm font-medium">
                                        @switch(request('price_range'))
                                            @case('under_2m') Dưới 2 triệu @break
                                            @case('2m_4m') Từ 2 - 4 triệu @break
                                            @case('4m_7m') Từ 4 - 7 triệu @break
                                            @case('7m_13m') Từ 7 - 13 triệu @break
                                            @case('13m_20m') Từ 13 - 20 triệu @break
                                            @case('over_20m') Trên 20 triệu @break
                                        @endswitch
                                    </span>
                                @endif

                                @if (request('sort'))
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-full bg-gray-100 text-gray-700 text-sm font-medium">
                                        @switch(request('sort'))
                                            @case('newest') Mới nhất @break
                                            @case('best_seller') Bán chạy @break
                    ...
                    @else
                        <div class="bg-white rounded-2xl border border-dashed border-gray-300 p-12 text-center">
                            <div class="w-20 h-20 mx-auto rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center mb-4">
